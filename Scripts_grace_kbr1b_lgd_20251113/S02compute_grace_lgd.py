...
class OrbitDataReader:
    ...
    def read_groops_integrated_fit2_ef_dynamicOrbit(self) -> GroopsOrbit:
        """读取GROOPS程序【99groopsIntegrateOrbitKBR.xml】计算的动力学轨道（轨道拟合两次再转换坐标），地心地固坐标系"""
        pattern1 = f'grace-c_integratedOrbitFitFit_EF_{self.date_str}_ggm05b_kbr.txt'
        pattern2 = f'grace-d_integratedOrbitFitFit_EF_{self.date_str}_ggm05b_kbr.txt'
    ...
    def read_groops_integrated_fit2_dynamicOrbit_ground_track(self) -> GroopsGroundTrack:
        """
        读取【99groopsIntegrateOrbitKBR.xml】程序处理后的大地坐标系轨道,使用GROOPS中的Orbit2Groundtracks模块进行了坐标转换成大地坐标系

        文件名如：grace-d_integratedOrbitFitFit_groundTrack_2020-07-07_ggm05b_kbr.txt
        该轨道坐标系为大地坐标系
        :return:
            data: [经度（deg）、纬度（deg）、高度（m）], [经度（deg）、纬度（deg）、高度（m）]
        """
        pattern1 = f'grace-c_integratedOrbitFitFit_groundTrack_{self.date_str}_ggm05b_kbr.txt'
        pattern2 = f'grace-d_integratedOrbitFitFit_groundTrack_{self.date_str}_ggm05b_kbr.txt'
...
class OrbitLoader(OrbitDataReader):
    ...
    def load_orbit_data(self, data_type, satellite='C', coord_type='cartesian') -> List[UnifiedOrbitData]:
        ...
        if data_type == 'groops_fit_eforbit':
            ...
            source_type = '99groopsIntegrateOrbitKBR.xml'
            ...
            source_type = 'Groops 【99groopsIntegrateOrbitKBR.xml】 GROUND TRACK'
...
class GraceLGDCalculator:
    ...
    def __init__(self, date_str, groops_workspace, trunc_st=0, trunc_ed=17280):
        self.date_str = date_str
    ...
    def read_groops_satelliteTracking_from_KBR1B(self) -> np.ndarray:
        """
        读取GROOPS软件【GraceL1b2SatelliteTracking】模块读取KBR数据得出的卫星跟踪数据（SST），采样间隔为5s

        :return: np.ndarray [MJD, biased_range, biased_range_rate, biased_range_acc]
        """
        filename = f"gracefo_satelliteTracking_{self.date_str}_from_KBR1B.txt"
        return self._read_data_file(os.path.join(self.groops_workspace,
                                                 fr'satellite_tracking_from_kbr//{self.date_str}',
                                                 filename))

    def read_groops_antCentr_from_KBR1B(self) -> np.ndarray:
        """
        读取GROOPS软件【GraceL1b2SatelliteTracking】模块读取KBR数据得出的天线相位偏移改正（ant_centr），对应KBR1B数据的9~11列，采样间隔为5s
        KBR数据的光时数据（lighttime），6~8列的含义分别为：
        ...
        """
        filename = f"gracefo_antCentr_{self.date_str}_from_KBR1B.txt"
        return self._read_data_file(os.path.join(self.groops_workspace,
                                                 f'satellite_tracking_from_kbr/{self.date_str}',
                                                 filename))

    def read_groops_lighttime_from_KBR1B(self) -> np.ndarray:
        """
        读取光时改正数据 (5s采样)
        ...
        """
        filename = f"gracefo_lighttime_{self.date_str}_from_KBR1B.txt"
        return self._read_data_file(os.path.join(self.groops_workspace,
                                                 f'satellite_tracking_from_kbr/{self.date_str}',
                                                 filename))
    ...
    def read_groops_satelliteTracking_from_dynamicOrbit(self) -> np.ndarray:
        """
        读取使用GROOPS软件 【99groopsIntegrateOrbitKBR.xml】模块处理KBR数据得出的卫星跟踪数据（SST），采样间隔为5s（重采样到5s）
        该模块计算的是动力学轨道
        惯性坐标系的轨道计算的结果

        :return: np.ndarray [MJD, range, range_rate, range_acc]
        """
        filename = f"grace-fo_satelliteTracking_{self.date_str}_ggm05b_kbr.txt"
        full_path = os.path.join(self.groops_workspace,
                                 "output", self.date_str,
                                 filename)
        return self._read_data_file(full_path)

    def correct_groops_satelliteTracking_from_KBR1B(self) -> Tuple[np.ndarray, ...]:
        """
        对KBR数据进行校正（包含光时和天线相位偏移改正）

        Reference：Gravity Recovery and Climate Experiment Follow-On (GRACE-FO) Level-1 Data Product User Handbook
            "Corrected biased range = biased range + lighttime corr (+ ant centr corr if KBR1B)
             Corrected range rate = range rate + lighttime rate (+ ant centr rate if KBR1B)
             Corrected range acceleration = range accl + lighttime accl (+ ant centr accl if KBR1B)"

        :return: np.ndarry [MJD, corrected_range, corrected_range_rate, corrected_range_acc]
        """
        kbr = self.read_groops_satelliteTracking_from_KBR1B()
        ant = self.read_groops_antCentr_from_KBR1B()
        light = self.read_groops_lighttime_from_KBR1B()

        # 验证时间戳一致性
        if not np.array_equal(kbr[:, 0], ant[:, 0]) or not np.array_equal(kbr[:, 0], light[:, 0]):
            raise ValueError("KBR, ANT和LIGHTTIME数据的时间戳不一致")

        # 应用校正
        corrected_range = kbr[:, 1] + light[:, 1] + ant[:, 1]
        corrected_rate = kbr[:, 2] + light[:, 2] + ant[:, 2]
        corrected_acc = kbr[:, 3] + light[:, 3] + ant[:, 3]

        return np.column_stack((kbr[:, 0], corrected_range, corrected_rate, corrected_acc))
    ...
    def compute_residual(self, reference_reader) -> np.ndarray:
        ...
        """
        # 获取校正后的KBR数据
        kbr_data = self.correct_groops_satelliteTracking_from_KBR1B()   # np.ndarray [MJD, range, range_rate, range_acc]

        # 获取参考轨道数据
        ref_data = reference_reader()   # np.ndarray [MJD, range, range_rate, range_acc]

        # 对齐数据（处理不同采样率）
        kbr_interval = np.median(np.diff(kbr_data[:, 0]))
        ref_interval = np.median(np.diff(ref_data[:, 0]))
        if np.isclose(ref_interval, kbr_interval):
            aligned_ref = ref_data
            aligned_kbr = kbr_data
        else:   # kbr降采样处理 (5s -> 10s)
            aligned_ref = ref_data
            aligned_kbr = kbr_data[::2]

        # 计算残差
        residuals = aligned_kbr[:, 2] - aligned_ref[:, 2]
        return np.column_stack((aligned_kbr[:, 0], residuals))

    def compute_sst_range_rate_residual_kbr1b_groops_dynamicOrbit(self) -> np.ndarray:
        """对KBR1B数据和GROOPS软件处理得到的动力学轨道得出的range-rate进行差分处理，计算residual range_rate(measurement - observation)"""
        return self.compute_residual(self.read_groops_satelliteTracking_from_dynamicOrbit)

    def res_rr_to_lgd(self, res_rr, fs=0.2):
    ...
    def compute_lgd_kbr1b_groopsOrbit(self):
        """
        计算KBR1B数据和GROOPS软件处理得到的动力学轨道得出的range-rate的LGD
        :return:
        """
        fs = 0.2  # KBR1B采样间隔为5s，频率为0.2
        output_dir = os.path.join(self.groops_workspace, 'results')
        data = self.compute_sst_range_rate_residual_kbr1b_groops_dynamicOrbit()  # 动力学轨道
    ...
    def run(self):
        ...
        self.compute_lgd_kbr1b_groopsOrbit()
