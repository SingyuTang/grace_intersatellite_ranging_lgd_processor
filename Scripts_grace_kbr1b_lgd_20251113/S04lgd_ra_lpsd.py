...
def lgd_ra_lasd(date_str, groops_workspace, data_type='ra', instrument='KBR'):
    """
    计算 LGD 时间序列的 LPSD 功率谱密度
    lgd和ra数据的单位是m/s^2，计算功率谱密度（PSD）的单位是m²/s⁴/Hz，振幅谱密度（ASD）的单位是m/s²/√Hz。本方法计算的是KBR-LGD的振幅谱密度。
    """
...
def run(start_date, end_date, groops_workspace, instrument='KBR'):
    """
    运行程序, 绘制ra和lgd的ASD
