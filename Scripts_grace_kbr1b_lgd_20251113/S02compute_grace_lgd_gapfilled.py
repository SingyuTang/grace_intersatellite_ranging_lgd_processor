...
class GraceGapLGDCalculator(GraceLGDCalculator):
    ...
    def correct_groops_satelliteTracking_from_KBR1B(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        return super().correct_groops_satelliteTracking_from_KBR1B()
    ...
    def compute_residual(self, reference_reader) -> np.ndarray:
        ...
        if aligned_ref.shape[0] != aligned_kbr.shape[0]:    # 当kbr星间测距数据采样不足17280时，对参考轨道数计算的数据进行对应位置的舍弃
            mask = np.isin(aligned_ref[:, 0], aligned_kbr[:, 0])
            aligned_ref = aligned_ref[mask]

        # 计算残差
        residuals = aligned_kbr[:, 2] - aligned_ref[:, 2]
        aligned_time = aligned_kbr[:, 0]

        def cubic_resample_to_full_day(aligned_time, residuals, freq=5):
            """对残差进行插值处理，使其与参考轨道数据的时间序列一致"""
            """
                使用三次样条插值将时间序列重采样为一天 17280 个采样点（2 秒间隔）
        ...
        if residuals.size != 17280:      # 当kbr星间测距数据采样不足17280时，对残差进行插值处理（三次样条插值）
            aligned_time = TimeConverter.mjd_to_datetime(aligned_kbr[:, 0])
    ...
    def compute_lgd_kbr1b_groopsOrbit(self):
        ...
        data = self.compute_sst_range_rate_residual_kbr1b_groops_dynamicOrbit()  # np.ndarray [aligned_time, residuals]
